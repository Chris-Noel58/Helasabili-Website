...
If production already has an enabled site, edit that active file instead of
creating a second server block. Verify the setting before reloading:
...
The `client_max_body_size` setting is required because Nginx rejects an
oversized upload before it reaches Django. Django stores larger uploads in
